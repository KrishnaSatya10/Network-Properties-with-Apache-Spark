...
def simple(g):
    ...
    # YOUR CODE HERE
    flipped_edges_df = g.edges.selectExpr('dst as src', 'src as dst')
    undirected_edges = g.edges.unionAll(
        flipped_edges_df).distinct().filter('src != dst')

    return GraphFrame(g.vertices, undirected_edges)
...
def degreedist(g):
    ...
    # YOUR CODE HERE
    degree_count = g.inDegrees.selectExpr(
        'id', 'inDegree as degree').groupBy('degree').count()
...
def readFile(filename, large, sqlContext=sqlContext):
    ...
    if large:
        ...
        lines = lines.mapPartitionsWithIndex(
            lambda ind, it: iter(list(it)[1:]) if ind == 0 else it)
    else:
        delim = ","
    ...
    lines_split = lines.map(lambda line: line.split(delim))

    graph_frame_edges = sqlContext.createDataFrame(lines_split, ['src', 'dst'])
    # graph_frame_edges.show(truncate=False)

    node_lines = lines_split.flatMap(lambda row: row).distinct()
    node_lines = node_lines.map(lambda x: Row(x))

    graph_frame_nodes = sqlContext.createDataFrame(node_lines, schema=['id'])
    # graph_frame_nodes.show(truncate=False)
    ...
    # graphframe vertices
    # YOUR CODE HERE
    g = GraphFrame(graph_frame_nodes, graph_frame_edges)
...
if len(sys.argv) > 1:
    ...
    print("Original graph has " + str(g.edges.count()) +
          " directed edges and " + str(g.vertices.count()) + " vertices.")
...
# Otherwise, generate some random graphs.
    ...
    eschema = StructType([StructField("src", IntegerType()),
    ...
    for gx in todo:
        ...
        v = sqlContext.createDataFrame(
            sc.parallelize(todo[gx].nodes()), vschema)
        e = sqlContext.createDataFrame(
            sc.parallelize(todo[gx].edges()), eschema)
